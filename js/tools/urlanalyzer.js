...
function analyze(raw) {
  ...
  if (url.protocol !== 'https:') {
    add(
      ...
      'The URL does not use HTTPS. Credentials or content could be exposed or modified in transit.'
    );
  } else {
    add(
      'good',
  ...
  if (h.tld && suspiciousTlds.has(h.tld)) {
    add(
      'medium',
      'Higher-risk / frequently abused TLD',
  ...
  if (
    ...
    add(
      'high',
      'Brand name appears in an unverified hostname',
  ...
  if (/%[0-9a-f]{2}/i.test(url.hostname)) {
    add(
      'high',
      'Percent-encoded hostname content',
  ...
  if (suspiciousPath) {
    add(
      'medium',
      'Sensitive action words in path',
  ...
  if (/\b(?:javascript|data|vbscript):/i.test(raw)) {
    add(
      'high',
      'Script/data URL scheme detected',
  ...
  if (redirectKeys.length) {
    add(
      'medium',
      'Redirect-style query parameter',
  ...
  if (
    ...
    add(
      'info',
      'Encoded characters can change how the URL appears',
  ...
  if (entireDecoded !== raw) {
    add(
      'info',
      'Percent-decoding changes the visible text',
  ...
  if (high > 0) {
    verdict = 'SUSPICIOUS';
    verdictClass = 'bad';
  } else if (medium >= 2 || warnings > 0) {
    verdict = 'CAUTION';
    verdictClass = 'warn';
  } else if (good > 0 && medium === 0) {
...
function renderResult(result) {
  ...
  const parts = [
    [
      'Scheme',
      u.protocol,
      'The protocol/scheme determines how the resource is addressed. HTTPS is generally expected for public web links.'
    ],
    [
      'Username',
      u.username,
      'Credentials before @ are unusual on modern web links and can be used to disguise the real hostname.'
    ],
    [
      'Password',
      u.password ? '••••••' : '',
      'Password components in URLs should be treated as sensitive.'
    ],
    [
      'Hostname',
      u.hostname,
      'The actual network host parsed by the browser. This is the most important domain component to inspect.'
    ],
    [
      'Port',
      u.port || '(default)',
      'Explicit non-standard ports deserve extra attention.'
    ],
    [
      'Path',
      u.pathname,
      'The resource/path requested from the host.'
    ],
    [
      'Query',
      u.search || '',
      'Parameters after ?. These can contain tracking, redirects, identifiers, or sensitive data.'
    ],
    [
      'Fragment',
      u.hash || '',
  ...
  const params = result.queryParams.length
    ? result.queryParams
        .map(
          k =>
            `<span class="tag mono">${escapeHtml(k)}</span>`
        )
        .join(' ')
  ...
  return `
    ...
    <section class="card">
      ...
      <p>
        ...
        a subdomain such as
        <span class="mono">login.example.com</span>
        is still controlled by
        <span class="mono">example.com</span>,
        whereas
        <span class="mono">example.com.evil.test</span>
        is controlled by
