...
`;
  ...
  function run() {
    ...
    if (!pattern) {
      out.hidden = false;
      out.innerHTML =
        '<p class="status warn">Enter a regular expression pattern.</p>';
      last = null;
      return;
    }
    ...
    try {
      ...
      out.innerHTML = `
        <p class="status danger">
          Invalid regular expression: ${escapeHtml(e.message)}
        </p>
      `;
      last = null;
      return;
                    ...
                    .map(
                      (m, i) => `
                        <tr>
                          ...
                          <td>
                            <code>
                              ${escapeHtml(JSON.stringify(m.groups))}
                            </code>
                          </td>
                        </tr>
                      `
